...
function Tribes({ campers, teams, generated, onGenerate, onMove, onAddCamper }) {
  ...
  const hasRosterFilter = hasTribeFilter || hasCabinFilter || hasGenderFilter || hasAgeFilter
  const filteredTeams = useMemo(() => teams.map((team) => ({
    ...team,
    visibleMembers: hasRosterFilter
      ? team.members.filter((member) => (!hasTribeFilter || matchesCamperQuery(member, tribeQuery)) && (!hasCabinFilter || normalizeIdentity(member.cabin) === normalizeIdentity(cabinFilter)) && (!hasGenderFilter || member.gender === genderFilter) && (!hasAgeFilter || String(member.age) === ageFilter))
      : team.members,
  })), [teams, tribeQuery, cabinFilter, genderFilter, ageFilter, hasTribeFilter, hasCabinFilter, hasGenderFilter, hasAgeFilter, hasRosterFilter])
  ...
  const selectedVisibleMembers = selected ? (hasRosterFilter ? selected.members.filter((member) => (!hasTribeFilter || matchesCamperQuery(member, tribeQuery)) && (!hasCabinFilter || normalizeIdentity(member.cabin) === normalizeIdentity(cabinFilter)) && (!hasGenderFilter || member.gender === genderFilter) && (!hasAgeFilter || String(member.age) === ageFilter)) : selected.members) : []
  ...
  const cabinRows = useMemo(() => teams.flatMap((team) => team.members.map((camper) => ({ camper, team })))
    .filter(({ camper }) => (!hasCabinFilter || normalizeIdentity(camper.cabin) === normalizeIdentity(cabinFilter)) && (!hasGenderFilter || camper.gender === genderFilter) && (!hasAgeFilter || String(camper.age) === ageFilter))
    .sort((a, b) => (a.camper.cabin || '').localeCompare(b.camper.cabin || '', 'es', { numeric: true }) || fullName(a.camper).localeCompare(fullName(b.camper), 'es')), [teams, cabinFilter, genderFilter, ageFilter, hasCabinFilter, hasGenderFilter, hasAgeFilter])
