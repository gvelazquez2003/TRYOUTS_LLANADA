...
const tribePrintRows = (members) => sortCampersForTribePrint(members)
  .map((member, index) => `<tr><td>${index + 1}</td><td>${escapeHtml(member.name)}</td><td>${escapeHtml(member.lastName || '—')}</td><td>${escapeHtml(member.age)}</td><td>${escapeHtml(member.cabin || '—')}</td></tr>`)
  .join('')
...
function downloadTribeSheet(team) {
  ...
  const rows = tribePrintRows(team.members)
...
function CamperForm({ camper, onSave, onClose }) {
  ...
  const submit = (event) => {
    ...
    const gender = normalizeStoredGender(form.gender)
...
function Tryouts({ campers, setCampers, onGoTribes }) {
  ...
  const updateCamperGender = (id, gender) => {
    setCampers((items) => items.map((item) => item.id === id ? { ...item, gender } : item))
  }
