...
import { BALANCE_DIMENSIONS, DEMO_CAMPERS, GENDER_OPTIONS, SKILLS, TRIBES, genderLabel, normalizeGender } from './data'
...
<body>
  ...
  const link = document.createElement('a')
  link.href = url
  link.download = `${cabinFilter ? `cabana-${fileSafe(cabinFilter)}` : groupFilter ? fileSafe(groupFilter) : 'campistas-por-cabana'}-tribus.html`
  document.body.appendChild(link)
  link.click()
  link.remove()
  window.setTimeout(() => URL.revokeObjectURL(url), 1000)
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Todas las tribus · Formación de Tribus</title>
  <style>
    ...
    .tribe-page { min-height: calc(100vh - 64px); padding: 32px; page-break-after: always; }
    .tribe-page:last-child { page-break-after: auto; }
    header { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 4px solid var(--tribe); padding-bottom: 16px; margin-bottom: 24px; }
    h1 { margin: 0; font-size: 34px; }
    .meta { text-align: right; color: #617168; font-size: 13px; }
    .flag { width: 58px; height: 38px; object-fit: cover; border-radius: 6px; margin-right: 12px; vertical-align: middle; box-shadow: 0 1px 4px rgba(0,0,0,.15); }
    table { width: 100%; border-collapse: collapse; margin-top: 18px; }
...
<body>
  <button class="no-print" onclick="window.print()">Imprimir todas las hojas</button>
  ${sections}
</body>
</html>`
  const blob = new Blob([html], { type: 'text/html;charset=utf-8' })
  ...
  const link = document.createElement('a')
  link.href = url
  link.download = 'todas-las-tribus.html'
  document.body.appendChild(link)
  link.click()
  link.remove()
  window.setTimeout(() => URL.revokeObjectURL(url), 1000)
...
function CamperForm({ camper, onSave, onClose }) {
  ...
  const submit = (event) => {
    ...
    onSave({ ...form, name, lastName, gender, age, cabin })
  }
...
function TribeCamperForm({ teams, initialTeamIndex = 0, onSave, onClose }) {
  const [form, setForm] = useState({ name: '', lastName: '', age: '', gender: '', cabin: '', teamIndex: initialTeamIndex, ...Object.fromEntries(SKILLS.map(({ key }) => [key, 3])) })
  const [error, setError] = useState('')
  const submit = (event) => {
    event.preventDefault()
    const name = form.name.trim()
    ...
    onSave({ ...form, name, lastName, gender, age, cabin }, teamIndex)
  }
  return <form onSubmit={submit}>
...
function ImportZone({ onImport }) {
  ...
  return <div className="import-block">
    <label className={`drop-zone ${dragging ? 'dragging' : ''}`} onDragOver={(event) => { event.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); load(event.dataTransfer.files[0]) }}>
      ...
      <span className="drop-icon"><UploadCloud size={23} /></span><span><strong>Arrastra aquí la lista de campistas</strong><small>CSV con Nombre, Apellido, Edad, Género (M/F), Cabaña, Fuerza, Velocidad, Inteligencia, Creatividad y Liderazgo</small></span><span className="button secondary compact"><FileSpreadsheet size={16} /> Elegir archivo</span>
    </label>
...
function Tryouts({ campers, setCampers, onGoTribes }) {
  ...
  const importCampers = ({ campers: incoming, hasCabin, hasScores, hasGender }) => {
    const seen = new Set()
    ...
    const mergeCamper = (current, imported) => ({
      ...
      age: imported.age,
      gender: hasGender ? (imported.gender || current.gender || '') : (current.gender || ''),
    ...
    setCampers((items) => {
      ...
      const additions = unique
        ...
        .map((camper) => ({ ...camper, id: crypto.randomUUID(), gender: hasGender ? camper.gender : '', cabin: hasCabin ? camper.cabin : '', ...(hasScores ? scoreValues(camper) : {}) }))
  ...
  return <>
    ...
    <section className="panel roster-panel"><div className="panel-header"><div><span className="eyebrow">Base de campistas</span><h2>Participantes</h2><p>Administra las fichas antes de hacer la división.</p></div><div className="panel-actions">{campers.length > 0 && <button className="button danger-action" onClick={deleteAll}><Trash2 size={18} /> Eliminar todos</button>}<button className="button primary" onClick={() => { setEditing(null); setModalOpen(true) }}><Plus size={18} /> Agregar campista</button></div></div>
      ...
      {campers.length ? <><div className="toolbar"><div className="search"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar campista o cabaña..." /></div><span>{filtered.length} de {campers.length}</span></div><div className="table-wrap"><table><thead><tr><th>Nombre</th><th>Apellido</th><th>Edad</th><th>Género</th><th>Cabaña</th><th>Programa</th>{SKILLS.map(({ key, label }) => <th key={key}>{label}</th>)}<th>Prom.</th><th /></tr></thead><tbody>{filtered.map((camper) => <tr key={camper.id}><td><span className="avatar">{initials(fullName(camper))}</span><strong>{camper.name}</strong></td><td><strong>{camper.lastName || '—'}</strong></td><td>{camper.age} años</td><td>{genderLabel(camper.gender)}</td><td><span className="cabin-pill">{camper.cabin || '—'}</span></td><td>{programLabel(getCabinProgram(camper.cabin))}</td>{SKILLS.map(({ key }) => <td key={key}><span className={`score score-${camper[key]}`}>{camper[key]}</span></td>)}<td><strong>{camperAverage(camper).toFixed(1)}</strong></td><td><div className="row-actions"><button onClick={() => { setEditing(camper); setModalOpen(true) }} title="Editar"><Edit3 size={17} /></button><button className="danger" onClick={() => setCampers((items) => items.filter(({ id }) => id !== camper.id))} title="Eliminar"><Trash2 size={17} /></button></div></td></tr>)}</tbody></table></div></> : <div className="empty-state"><span><UserPlus size={34} /></span><p>Agrega el primer campista, arrastra un CSV o carga un grupo de ejemplo.</p><div><button className="button primary" onClick={() => setModalOpen(true)}><Plus size={18} /> Agregar campista</button><button className="button secondary" onClick={addDemo}>Cargar datos de prueba</button></div></div>}
    </section>
...
function Tribes({ campers, teams, generated, onGenerate, onMove, onAddCamper }) {
  ...
  const hasGenderFilter = generated && genderFilter.trim().length > 0
  const hasAgeFilter = generated && ageFilter.trim().length > 0
  const hasProgramFilter = generated && programFilter.trim().length > 0
  const hasSideFilter = generated && sideFilter !== 'all'
  const hasRosterFilter = hasTribeFilter || hasCabinFilter || hasGenderFilter || hasAgeFilter || hasProgramFilter
  const filteredTeams = useMemo(() => teams.map((team) => ({
    ...team,
    visibleMembers: hasRosterFilter
      ? team.members.filter((member) => (!hasTribeFilter || matchesCamperQuery(member, tribeQuery)) && (!hasCabinFilter || normalizeIdentity(member.cabin) === normalizeIdentity(cabinFilter)) && (!hasGenderFilter || member.gender === genderFilter) && (!hasAgeFilter || String(member.age) === ageFilter) && (!hasProgramFilter || getCabinProgram(member.cabin) === programFilter))
      : team.members,
  })), [teams, tribeQuery, cabinFilter, genderFilter, ageFilter, programFilter, hasTribeFilter, hasCabinFilter, hasGenderFilter, hasAgeFilter, hasProgramFilter, hasRosterFilter])
  ...
  const selectedVisibleMembers = selected ? (hasRosterFilter ? selected.members.filter((member) => (!hasTribeFilter || matchesCamperQuery(member, tribeQuery)) && (!hasCabinFilter || normalizeIdentity(member.cabin) === normalizeIdentity(cabinFilter)) && (!hasGenderFilter || member.gender === genderFilter) && (!hasAgeFilter || String(member.age) === ageFilter) && (!hasProgramFilter || getCabinProgram(member.cabin) === programFilter)) : selected.members) : []
  ...
  const ageOptions = useMemo(() => Array.from(new Set(campers.map((camper) => camper.age).filter(Boolean))).sort((a, b) => a - b), [campers])
  const cabinRows = useMemo(() => teams.flatMap((team) => team.members.map((camper) => ({ camper, team })))
    .filter(({ camper }) => (!hasCabinFilter || normalizeIdentity(camper.cabin) === normalizeIdentity(cabinFilter)) && (!hasGenderFilter || camper.gender === genderFilter) && (!hasAgeFilter || String(camper.age) === ageFilter) && (!hasProgramFilter || getCabinProgram(camper.cabin) === programFilter))
    .sort((a, b) => (a.camper.cabin || '').localeCompare(b.camper.cabin || '', 'es', { numeric: true }) || fullName(a.camper).localeCompare(fullName(b.camper), 'es')), [teams, cabinFilter, genderFilter, ageFilter, programFilter, hasCabinFilter, hasGenderFilter, hasAgeFilter, hasProgramFilter])
  ...
  return <>
    ...
    <section className="weights-panel"><div><span className="eyebrow">Criterios de balance</span><strong>Importancia relativa</strong></div><div className="weight-chips"><span>Edad + Velocidad <b>40%</b></span><span>Liderazgo <b>20%</b></span><span>Fuerza <b>20%</b></span><span>Creatividad <b>10%</b></span><span>Inteligencia <b>10%</b></span></div><small>La generación automática equilibra cantidades, género, programa y cada programa por género: Bosque, Sabana, AV y CIT; después puedes modificar manualmente sin restricciones.</small></section>
