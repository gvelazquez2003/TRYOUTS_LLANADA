...
export const LOCAL_STORAGE_KEYS = {
  ...
  seasonSettings: 'formacion-tribus-season-settings-v1',
}
...
export function readLocalSnapshot() {
  try {
    return {
      ...
      seasonSettings: JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEYS.seasonSettings)) || null,
    }
  } catch {
    return { campers: [], assignments: null, seasonSettings: null }
  }
}

export function saveLocalSnapshot({ campers, assignments, seasonSettings }) {
  ...
  if (seasonSettings) localStorage.setItem(LOCAL_STORAGE_KEYS.seasonSettings, JSON.stringify(seasonSettings))
  else localStorage.removeItem(LOCAL_STORAGE_KEYS.seasonSettings)
}
...
export async function readRemoteSnapshot() {
  ...
  return {
    ...
    seasonSettings: row.season_settings || null,
    updatedAt: row.updated_at,
    updatedBy: row.updated_by,
  }
}

export async function writeRemoteSnapshot({ campers, assignments, seasonSettings }) {
  if (!isRemoteSyncConfigured()) return null
  const row = {
    id: SYNC_ROOM,
    campers,
  ...
  const save = (payloadRow) => fetch(syncUrl('?on_conflict=id'), {
    method: 'POST',
    headers: syncHeaders({ Prefer: 'resolution=merge-duplicates,return=representation' }),
    body: JSON.stringify([payloadRow]),
  })

  let response = await save({ ...row, season_settings: seasonSettings || null })
  if (!response.ok) {
    const details = await response.text()
    if (response.status === 400 && details.toLowerCase().includes('season_settings')) response = await save(row)
    else throw new Error(`Guardado en Supabase fallÃ³ (${response.status}). ${details.slice(0, 220)}`)
  }
  if (!response.ok) throw await responseError(response, 'Guardado en Supabase')
  const rows = await response.json()
  const savedRow = rows[0]
  return savedRow ? { updatedAt: savedRow.updated_at, updatedBy: savedRow.updated_by } : null
}
